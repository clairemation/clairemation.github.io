...
function processNormalsImage() {
  return new Promise(function (resolve, reject) {
    var data = function getImageData() {
      ...
      document.getElementById('scratch').getContext('2d').drawImage(image, startx, starty, sourceWidth, sourceHeight, 0, 0, sourceWidth, sourceHeight);
...
function tick(dt) {
  ...
  if (!(ballY >= winHeight || ballX >= winWidth)) {
  ...
  if ($([newBall[0], newBall[1]]).distance([256, 256]).$ <= 255) {
    ballX = newBall[0];
    ballY = newBall[1];
    ballZ = newBall[2];
